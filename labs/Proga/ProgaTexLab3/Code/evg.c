...
// Новая производная функции
double df(double x) {
    return 3 * pow(x, 2) - 4 * x + 3;
}

double pervf(double x) {
    return pow(x, 4)/4 - 2*pow(x, 2)/3 + 3*pow(x, 2)/2 + 4*x;
}

// Метод средних прямоугольников
double integ(double a, double b, int n) {
    double dx = (b - a) / n; // разбиение отрезка [a, b] на n равных частей
    double sum = 0.0; // инициализация суммы площадей прямоугольников

    for (int i = 0; i < n; i++) {
        double x_mid = a + (i + 0.5) * dx; // координата x середины прямоугольника
        double y_mid = f(x_mid); // значение функции в середине прямоугольника
        sum += y_mid * dx; // площадь i-го прямоугольника
    }
