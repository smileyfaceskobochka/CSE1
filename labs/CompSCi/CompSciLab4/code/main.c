...
void one(int x, int n) {
    printf("1. ");
    if (x <= -(1 << (n - 1)) || x >= (1 << (n - 1))) {
        print_error("число не входит в диапазон представления в прямом коде для сетки", x, n);
        return;
    }
...
char* two(int x, int n) {
    ...
    if (x < -(1 << (n - 1)) || x >= (1 << (n - 1))) {
        print_error("число выходит из диапазона представления в дополнительном коде для сетки", x, n);
        free(arr);
        return NULL;
    }
